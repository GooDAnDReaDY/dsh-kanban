...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const helpers = {
      neighboursFor, tasksOf, cardRef, cardStatus, planLine, stateLabel, isCollapsed, normalizeBoard, reposOf, errorKey,
    ...
    function BoardScreen(props) {
      ...
      const bar = React.createElement('div', { className: 'dkb-bar' },
        ...
        boards.length > 1
          ? React.createElement('select', {
            ...
            onChange: (e) => { setOpenTask(null); setCollapsed({}); setBoard(e.target.value) },
      ...
      const columns = React.createElement('div', { className: 'dkb-cols' },
        state.columns.map((col) => {
          ...
          const shut = isCollapsed(col, collapsed)
          const toggle = () => setCollapsed((prev) => Object.assign({}, prev, { [col.id]: !shut }))
          const count = col.limit !== undefined && col.limit !== null
