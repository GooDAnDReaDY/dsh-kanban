...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const helpers = {
      neighboursFor, tasksOf, cardRef, cardStatus, planLine, stateLabel, normalizeBoard, reposOf, errorKey,
    ...
    const css =
      ...
      '.dkb-panel{position:absolute;top:0;right:0;bottom:0;width:min(420px,92%);background:var(--dsw-alias-bg-layer-2);border-left:1px solid var(--dsw-alias-border-l2);padding:16px 16px 84px;box-sizing:border-box;overflow-y:auto;display:flex;flex-direction:column;gap:12px;z-index:70}' +
    ...
    const en = {
      ...
      'field.pushToGitea': 'Write column labels back to Gitea',
      'field.webhookSecretRef': 'Webhook credential name',
      'hint.pushToGitea': 'Moving a card sets its column label; Done also closes the issue.',
    ...
    const ru = {
      ...
      'field.pushToGitea': 'Переносить метки колонок в Gitea',
      'field.webhookSecretRef': 'Имя учётной записи для вебхука',
      'hint.pushToGitea': 'Перенос карточки ставит метку колонки; Done дополнительно закрывает issue.',
      ...
      return React.createElement('div', {
        ...
        ref || waiting
          ? React.createElement('div', { className: 'dkb-taskRef' },
            ...
            ref ? React.createElement('span', null, ref) : null)
          : null,
    ...
    function BoardScreen(props) {
      ...
      const load = React.useCallback(async (nextRepo, nextBoard) => {
        try {
          const query = '?board=' + encodeURIComponent(nextBoard || 'main')
            + (nextRepo ? '&repo=' + encodeURIComponent(nextRepo) : '')
      ...
      React.useEffect(() => { load(repo, board) }, [load, repo, board])
      ...
      const drop = React.useCallback((column, index) => {
        ...
        if (task.column === column) { commitMove(task, column, where); return }
        // Перенос в другую колонку запускает работу. Мышь слишком легко
        // задевает карточку, чтобы такое уходило без спроса.
        setDialog({ mode: 'move', task, column, where })
      }, [state, commitMove])
      ...
      const bar = React.createElement('div', { className: 'dkb-bar' },
        ...
        boards.length > 1
          ? React.createElement('select', {
            ...
            onChange: (e) => { setOpenTask(null); setBoard(e.target.value) },
      ...
      return React.createElement('div', { className: 'dkb-screen' },
        ...
        columns, panel, dialogEl, moveDialogEl,
      )
    }
    ...
    function apply(ctx) {
      ...
      const chipSlot = registerFirst(ctx, [
        { name: 'conversation.session.header.utilities', id: '@goodandready-private/dsh-kanban.chip', order: 26, locale: NS, inject: () => ({ ctx }) },
      ], TaskChip)
