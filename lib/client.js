  ...
  factory: (require) => {
    ...
    function KanbanSettingsCard(props) {
      ...
      const scope = React.useMemo(
        () => {
          const s = (ctx?.get && ctx.get('lanSettings')) || ctx?.settingsScope
          return s ? s.bind({ namespace: NS }) : undefined
        },
        [ctx],
      )
      ...
      if (!open) return React.createElement('li', { className: 'dkb-card' }, head)
      ...
      if (status === 'loading') {
        ...
        for (const grp of groups) {
          ...
          for (const spec of fieldsInGroup) {
            if (spec.kind === 'bool') {
              fieldElements.push(React.createElement('div', { className: 'dkb-field', key: spec.field },
          ...
          rows.push(React.createElement('div', { className: 'dkb-section-card', key: 'g-' + grp },
            React.createElement('div', { className: 'dkb-section-title' },
              React.createElement('span', null, t('group.' + grp)),
              badge,
            ),
            ...fieldElements,
          ))
      ...
      return React.createElement('li', { className: 'dkb-card dkb-cardOpen' },
        head, React.createElement('div', { className: 'dkb-body' }, body))
    }
    ...
    function TaskCard(props) {
      ...
      return React.createElement('div', {
        ...
        React.createElement('div', { className: 'dkb-taskHead' },
          ...
          taskRef(task, t),
          task.cron ? React.createElement('span', { className: 'dkb-tag', title: task.cron }, '⏰ cron') : null,
          task.progressDump ? React.createElement('span', { className: 'dkb-tag', title: 'PROGRESSDUMP' }, '📋 dump') : null,
          task.rejectReason ? React.createElement('span', { className: 'dkb-tag', style: { background: 'var(--dsw-alias-state-danger-bg, #ffebe9)', color: 'var(--dsw-alias-state-danger-primary, #cf222e)', borderColor: 'var(--dsw-alias-state-danger-border, #ffc1c0)' }, title: task.rejectReason }, '↩ ' + t('acceptance.rejectedBadge')) : null,
          Array.isArray(task.checklist) && task.checklist.length > 0
            ? React.createElement('span', { className: 'dkb-tag', title: 'DoD Checklist' }, `☑ ${task.checklist.filter((i) => i.completed).length}/${task.checklist.length}`)
            : null,
          task.report && task.report.summary
            ? React.createElement('span', { className: 'dkb-tag', title: 'Execution Report' }, '📄 report')
            : null,
          Array.isArray(task.comments) && task.comments.length > 0
            ? React.createElement('span', { className: 'dkb-tag', title: 'Comments' }, `💬 ${task.comments.length}`)
            : null,
    ...
    function BoardScreen(props) {
      ...
      // Подписка на Server-Sent Events (SSE) для мгновенного обновления доски (#222).
      React.useEffect(() => {
        ...
        const connect = () => {
          try {
            ...
            es.addEventListener('board:change', () => {
              load(board)
            })
      ...
      const panel = openTask
        ? React.createElement('div', {
          ...
          React.createElement('div', { className: 'dkb-row' },
            React.createElement('label', { className: 'dkb-label' }, t('reasoning.label')),
            React.createElement('select', {
              ...
              React.createElement('option', { value: '' }, t('reasoning.default')),
              React.createElement('option', { value: 'low' }, t('reasoning.low')),
              React.createElement('option', { value: 'medium' }, t('reasoning.medium')),
              React.createElement('option', { value: 'high' }, t('reasoning.high')),
            ),
          ),
          React.createElement('div', { className: 'dkb-row' },
          ...
          React.createElement('div', { className: 'dkb-note' },
            ...
            openTask.issueUrl
              ...
              : null),
          openTask.worktree
            ? React.createElement('div', { className: 'dkb-note', style: { wordBreak: 'break-all' } },
          ...
          openTask.permission && !openTask.permissionConfirmedAt
            ? React.createElement('div', {
                ...
                React.createElement('div', { style: { marginBottom: 6, fontSize: 13, fontWeight: 500, color: 'var(--dsw-alias-state-warn-primary, #856404)' } },
          ...
          React.createElement('div', { className: 'dkb-edit' },
            ...
            React.createElement('div', { className: 'dkb-field' },
              React.createElement('span', { className: 'dkb-label' }, t('task.cron')),
              React.createElement('input', {
          ...
          openTask.progressDump
            ? React.createElement('div', null,
                React.createElement('div', { className: 'dkb-group' }, t('task.progressDump')),
          ...
          openTask.runs && openTask.runs.length
            ? React.createElement('div', null,
                React.createElement('div', { className: 'dkb-group' }, t('task.runs')),
                React.createElement('ul', { className: 'dkb-log' }, openTask.runs.map((r) => React.createElement('li', {
          ...
          openTask.worktree
            ? React.createElement('div', {
                ...
                React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
                  React.createElement('div', { className: 'dkb-group', style: { margin: 0 } },
                    t('diff.title'),
                ...
                diffExpanded ? React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 6 } },
                  commitsData.length > 0
                    ? React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 4 } },
                        commitsData.map((cm) => React.createElement('div', {
                          key: cm.hash,
                          style: { fontSize: 11, display: 'flex', gap: 6, opacity: 0.85, fontFamily: 'monospace' },
                        },
                          React.createElement('span', { style: { color: 'var(--dsw-alias-label-link, #3b82f6)' } }, cm.hash),
                          React.createElement('span', { style: { flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' } }, cm.subject),
                          React.createElement('span', { style: { opacity: 0.6 } }, cm.date),
                        ))
                      )
                    : null,
                  diffLoading
                    ? React.createElement('div', { style: { fontSize: 12, opacity: 0.7 } }, 'Loading diff...')
                    : diffData?.diff
                      ? React.createElement('pre', {
                          style: {
                      ...
                      : React.createElement('div', { style: { fontSize: 12, opacity: 0.7 } }, t('diff.empty'))
                ) : null
              )
            : null,
          openTask.column === 'review'
            ? React.createElement('div', {
                ...
                React.createElement('div', { className: 'dkb-group', style: { margin: 0 } }, t('acceptance.title')),
                React.createElement('div', { style: { display: 'flex', gap: 8 } },
                  openTask.worktree
                    ? React.createElement('button', {
                        type: 'button', className: 'dkb-save',
                        disabled: merging,
                        style: { background: 'var(--dsw-alias-bg-success, #16a34a)', color: '#fff' },
                        onClick: async () => {
                          try {
                            setMerging(true)
                ...
                rejectModalOpen
                  ? React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6 } },
                      React.createElement('span', { className: 'dkb-label' }, t('acceptance.rejectTitle')),
          ...
          React.createElement('div', { style: { margin: '4px 0' } },
            React.createElement('div', { className: 'dkb-group', style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
              React.createElement('span', null, t('checklist.title')),
              Array.isArray(openTask.checklist) && openTask.checklist.length > 0
                ? React.createElement('span', { className: 'dkb-hint', style: { fontWeight: 'normal' } },
            ...
            Array.isArray(openTask.checklist) && openTask.checklist.length > 0
              ? React.createElement('ul', { style: { listStyle: 'none', padding: 0, margin: '4px 0', display: 'flex', flexDirection: 'column', gap: 4 } },
                  openTask.checklist.map((item, idx) => React.createElement('li', {
                    key: idx,
                    style: {
                    ...
                    React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } },
                      ...
                      item.required !== false
                        ? React.createElement('span', { className: 'dkb-tag', style: { fontSize: 10 } }, t('checklist.required'))
                        : null,
                    ),
                    item.evidence
                      ? React.createElement('div', {
              ...
              : React.createElement('p', { className: 'dkb-hint' }, t('checklist.empty')),
          ...
          openTask.report && openTask.report.summary
            ? React.createElement('div', {
                ...
                React.createElement('div', { className: 'dkb-group', style: { margin: '0 0 6px 0' } }, t('report.title')),
                React.createElement('div', { style: { fontSize: 13, color: 'var(--dsw-alias-label-primary)', marginBottom: 6, fontWeight: 500 } },
                ...
                openTask.report.changedFiles && openTask.report.changedFiles.length
                  ? React.createElement('div', { style: { fontSize: 12, marginBottom: 4 } },
                      React.createElement('span', { style: { fontWeight: 600, color: 'var(--dsw-alias-label-secondary)' } }, t('report.changedFiles') + ': '),
                      openTask.report.changedFiles.map((f) => React.createElement('span', { className: 'dkb-tag', key: f, style: { margin: '1px 2px', fontFamily: 'monospace' } }, f)),
                    )
                  : null,
                openTask.report.checksRun && openTask.report.checksRun.length
                  ? React.createElement('div', { style: { fontSize: 12, marginBottom: 4 } },
                      React.createElement('span', { style: { fontWeight: 600, color: 'var(--dsw-alias-label-secondary)' } }, t('report.checksRun') + ': '),
                      openTask.report.checksRun.map((chk, i) => React.createElement('div', { key: i, style: { fontFamily: 'monospace', fontSize: 11, marginLeft: 8 } }, '• ' + chk)),
                    )
                  : null,
                openTask.report.artifacts && openTask.report.artifacts.length
                  ? React.createElement('div', { style: { fontSize: 12, marginBottom: 4 } },
                      React.createElement('span', { style: { fontWeight: 600, color: 'var(--dsw-alias-label-secondary)' } }, t('report.artifacts') + ': '),
                      openTask.report.artifacts.map((a, i) => React.createElement('span', { className: 'dkb-tag', key: i, style: { margin: '1px 2px' } }, a)),
                    )
                  : null,
                openTask.report.risks && openTask.report.risks.length
                  ? React.createElement('div', { style: { fontSize: 12 } },
                      React.createElement('span', { style: { fontWeight: 600, color: 'var(--dsw-alias-label-secondary)' } }, t('report.risks') + ': '),
                      openTask.report.risks.map((r, i) => React.createElement('div', { key: i, style: { fontSize: 11, color: 'var(--dsw-alias-state-warn-primary, #b08800)', marginLeft: 8 } }, '⚠ ' + r)),
                    )
                  : null,
              )
            : null,
          React.createElement('div', { style: { margin: '6px 0' } },
            React.createElement('div', { className: 'dkb-group', style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
              React.createElement('span', null, t('comments.title')),
              Array.isArray(openTask.comments) && openTask.comments.length > 0
                ? React.createElement('span', { className: 'dkb-hint' }, `(${openTask.comments.length})`)
                : null,
            ),
            Array.isArray(openTask.comments) && openTask.comments.length > 0
              ? React.createElement('ul', {
                  ...
                  openTask.comments.map((c, i) => React.createElement('li', {
                    key: c.id || i,
                    style: {
                    ...
                    React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
                      ...
                      React.createElement('span', { style: { color: 'var(--dsw-alias-label-tertiary)', fontSize: 11 } },
                        new Date(c.createdAt || Date.now()).toLocaleTimeString()),
                    ),
                    React.createElement('div', { style: { whiteSpace: 'pre-wrap', color: 'var(--dsw-alias-label-primary)' } }, c.text),
                  )))
              : React.createElement('p', { className: 'dkb-hint' }, t('comments.empty')),
          ...
          React.createElement('div', { className: 'dkb-foot dkb-footSplit' },
            ...
            (openTask.worktree || openTask.rejectReason || (Array.isArray(openTask.runs) && openTask.runs.length > 0)) ? React.createElement('button', {
              type: 'button',
              className: 'dkb-secondary',
              style: { marginRight: '8px', border: '1px solid var(--dsw-alias-border-color-primary, #444)' },
    ...
    function apply(ctx) {
      ...
      // Карточка настроек в Настройки → Плагины → Настройки плагинов.
      // Ключ обязан совпадать с пространством настроек: вкладка перебирает
      // объявленные пространства и рисует слот settings.plugin.item с entryKey,
      // равным имени пространства.
      // Собственный раздел верхнего уровня (settings.section) не регистрируется,
      // чтобы не засорять плоский общий список настроек (#238).
      refreshMirrorUntilVisible(ctx)
      const SettingsErrorBoundary = createErrorBoundary()
      const SafeSettingsCard = (props) => React.createElement(SettingsErrorBoundary, null, React.createElement(KanbanSettingsCard, props))
