...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const helpers = {
      ...
      chipSessionId, matchesQuery, splitFullName, COLUMN_ORDER,
    ...
    const css =
      ...
      '.dkb-entry{width:100%;height:32px;color:var(--dsw-alias-label-secondary);cursor:pointer;white-space:nowrap;background:0 0;border:none;border-radius:8px;align-items:center;gap:8px;padding:0 12px;font-size:13px;display:flex}' +
      '.dkb-entry:hover{background:var(--dsw-specific-sidebar-nav-item-hover);color:var(--dsw-alias-label-primary)}' +
      '.dkb-entry[data-active]{background:var(--dsw-specific-sidebar-nav-item-active);color:var(--dsw-alias-label-primary);font-weight:600}' +
      ...
      '[data-dsh-frame][data-sidebar-collapsed] .dkb-entry{justify-content:center;width:100%;padding:0}' +
      ...
      // Сама доска
      '.dkb-screen{box-sizing:border-box;background:var(--dsw-alias-bg-base);min-width:0;height:100%;min-height:0;color:var(--dsw-alias-label-primary);font-family:var(--dsw-font-family);flex-direction:column;gap:12px;padding:14px 16px 16px;display:flex}' +
      ...
      '.dkb-cols{overscroll-behavior-inline:contain;scrollbar-color:var(--dsw-alias-border-l3) var(--dsw-alias-interactive-bg-hover);scrollbar-width:thin;flex:1;grid-auto-columns:minmax(220px,1fr);grid-auto-flow:column;gap:12px;min-height:0;padding-bottom:6px;display:grid;overflow:auto hidden}' +
      ...
      '.dkb-col{background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);border-radius:12px;flex-direction:column;min-height:0;display:flex;overflow:hidden}' +
      '.dkb-colOver{border-color:var(--dsw-alias-border-l3)}' +
      '.dkb-colHead{flex:none;align-items:center;gap:6px;padding:10px 12px;display:flex}' +
      ...
      '.dkb-tag{white-space:nowrap;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-interactive-bg-hover);border-radius:999px;padding:2px 6px;font-size:12px;line-height:1}' +
      '.dkb-panel{position:absolute;top:0;right:0;bottom:0;width:min(420px,92%);background:var(--dsw-alias-bg-layer-2);border-left:1px solid var(--dsw-alias-border-l2);padding:16px;box-sizing:border-box;overflow-y:auto;display:flex;flex-direction:column;gap:12px;z-index:70}' +
    ...
    function BoardScreen(props) {
      ...
      const bar = React.createElement('div', { className: 'dkb-bar' },
        ...
        React.createElement('button', { type: 'button', className: 'dkb-save', onClick: () => setDialog({ mode: 'own', title: '', full: '', repos: null, issues: null, error: '' }) }, t('board.newTask')),
      )
      ...
      const columns = React.createElement('div', { className: 'dkb-cols' },
        state.columns.map((col) => {
          const all = tasksOf(state.tasks, col.id)
          ...
          return React.createElement('div', {
            ...
            React.createElement('div', { className: 'dkb-colHead' },
              ...
              React.createElement('span', {
                ...
                ? all.length + ' / ' + t('board.limit', { n: col.limit })
                : String(all.length)),
            ),
              ...
              : React.createElement('div', null,
                React.createElement('div', { className: 'dkb-row' },
                  React.createElement('select', {
                    ...
                    (dialog.repos || []).map((r) => React.createElement('option', {
                      value: r.fullName || (r.owner + '/' + r.repo), key: r.fullName || (r.owner + '/' + r.repo),
                    }, r.fullName || (r.owner + '/' + r.repo))),
                  ),
                ),
                      ...
                      React.createElement('button', {
                        ...
                        onClick: async () => {
                          ...
                          try {
                            await api('/import', {
                              method: 'POST',
                              body: JSON.stringify({ owner: pair.owner, repo: pair.repo, issueNumber: issue.number }),
                            })
                            setDialog(null)
                            await load(repo)
                          } catch (e) {
    ...
    function sidebarRoot() {
      ...
      return (logo && logo.parentElement) || column.firstElementChild || undefined
    }

    /** Кнопка новой сессии: якорь, сразу за которым встаёт наша строка. */
    function anchorButton(root) {
    ...
    function createEntry(toggle, label) {
      const entry = document.createElement('button')
      entry.type = 'button'
      entry.setAttribute(ENTRY_ATTR, '')
      entry.setAttribute('data-dsh-plugin', 'kanban')
      entry.setAttribute('data-dsh-part', 'sidebar-entry')
      entry.className = 'dkb-entry'
      entry.setAttribute('aria-label', label)
      entry.innerHTML = '<span class="dkb-entryIcon">' + ICON + '</span>'
        + '<span class="dkb-entryLabel"></span>'
      const text = entry.querySelector('.dkb-entryLabel')
      if (text !== null) text.textContent = label
      entry.addEventListener('click', () => toggle.toggle())
      return entry
    }

    /** Вставить строку сразу за строкой новой сессии. */
    function placeEntry(root, entry) {
      const button = anchorButton(root)
    ...
    function mountBoard(ctx, toggle) {
      ...
      const ensure = () => {
        ...
        root.render(React.createElement(BoardScreen, { ctx, onClose: () => toggle.set(false) }))
      }
