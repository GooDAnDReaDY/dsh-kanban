...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const helpers = {
      neighboursFor, tasksOf, cardRef, taskRef, cardStatus, facetLabel, boardTitle, planLine, stateLabel, isCollapsed,
      agoText, exactAt, syncLine, readableOn, tagStyle, priorityColor, renderBlocks, renderSpans, matchesFilters, anySelected, toggleValue, normalizeBoard, errorKey,
    ...
    function BoardScreen(props) {
      ...
      const columns = React.createElement('div', { className: 'dkb-cols' },
        state.columns.map((col) => {
          ...
          let group = null
          const sizes = new Map()
          for (const x of items) sizes.set(x.repo || '', (sizes.get(x.repo || '') || 0) + 1)
          ...
          items.forEach((task, i) => {
            ...
            if (manyGroups && repo !== group) {
              ...
              rows.push(React.createElement('button', {
                ...
                React.createElement('span', null, repo || t('board.noRepo')),
