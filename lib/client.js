...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const helpers = {
      neighboursFor, tasksOf, cardRef, taskRef, cardStatus, facetLabel, boardTitle,
    ...
    function BoardScreen(props) {
      ...
      React.useEffect(() => {
        ...
        api('/presets')
          .then((out) => {
            if (!alive || !out) return
            setPresets({ agentPresets: out.agentPresets || [], access: out.access || [] })
            // Пустое значение — «как у харнесса по умолчанию»; подставляем его
            // же явно, чтобы человек видел, с чем задача пойдёт.
            setAgentPreset(out.agentPresetDefault || '')
            setPermission(out.accessDefault || '')
          })
          .catch(() => {})
        return () => { alive = false }
      }, [])
