...
export const CONFIG_HINTS = {
  ...
  giteaOwner: 'Организация Gitea, чьи репозитории смотрит доска. Пусто — определяется сама, если организация одна.',
  columnNames: 'Свои названия колонок парами «идентификатор=Название» через запятую. Пусто — обычные названия.',
  watchRepos: 'Сузить подхват до этих репозиториев, через запятую. Пусто — все репозитории организации, где есть открытые задачи.',
  archiveAfterDays: 'Через сколько дней в колонке «Выполнено» карточка уходит в архив. Ноль — не убирать.',
  ...
  worktreeIsolation: 'Изолировать сессии задач в отдельные git worktrees для предотвращения коллизий.',
  worktreeRoot: 'Каталог для воркдеревьев задач. Пусто — $DSH_HOME/worktrees или .worktrees проекта.',
  cronEnabled: 'Включить фоновый запуск задач по расписанию cron на стороне хоста.',
  preventIdleSleep: 'Запретить переход хоста в спящий режим, пока есть активные сессии задач.',
  progressDumpEnabled: 'Включить поддержку фиксации срезов PROGRESSDUMP для передачи эстафеты между сессиями.',
  permissionGateEnabled: 'Требовать подтверждения человеком для запуска задач с повышенными правами.',
  sessionDefaultPermission: 'Базовый уровень прав без подтверждения. Права выше этого требуют одобрения.',
  announceToAgent: 'Добавлять секцию о доске задач в системный промпт агентов.',
  autoSyncSessions: 'Автоматически создавать карточки задач для обычных сессий чата DSH.',
}
