...
export function apply(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (req.method === 'POST' && action === 'refresh') {
        reply(res, await refreshTask({ gitea: ctx.get('gitea'), store, id }))
        return
      }
  ...
  // обязаны внятно объяснять причину, а не отвечать 404. Читаем через get, а
  // не свойством: обращение к незарегистрированной службе свойством —
  // ловушка контекста cordis.
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/dsh-kanban/gitea/repos',
    handler: async (req, res) => {
      const url = urlOf(req)
      reply(res, await searchRepos({ gitea: ctx.get('gitea'), query: url.searchParams.get('q') || '' }))
    },
