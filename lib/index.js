...
export function apply(ctx, config) {
    ...
    if (cfg.announceToAgent !== false && sp && typeof sp.section === 'function') {
      try {
        disposePromptSection = sp.section({
          ...
          order: 200,
          text: 'В системе развернута доска задач dsh-kanban. При работе над задачами вы можете перемещать карточки инструментом board_move и вести чек-лист плана через board_plan. При исчерпании контекста или завершении подэтапа сформируйте блок <<<PROGRESSDUMP\\nЦель: ...\\nПрогресс: ...\\nСледующие шаги: ...\\n>>>PROGRESSDUMP для передачи эстафеты следующей сессии.',
        })
      } catch (err) {
        ctx.logger?.warn?.(`dsh-kanban: не удалось зарегистрировать секцию промпта: ${err?.message}`)
      }
    }
  }
  ...
  }, 'dsh-kanban: секция системного промпта')
    ...
    try {
      ...
      if (
        ...
        if (!existingTask && (event?.type === 'turn/start' || event?.type === 'session/created' || event?.type === 'input/user' || event?.type === 'message')) {
          ...
          const sessionTitle = session.title || session.name || (userText ? userText.slice(0, 50) : `Диалог ${sessionIdStr.slice(0, 8)}`)
          const createdTask = store.createTask({
            title: sessionTitle,
            body: userText || `Автоматически создано из сессии чата ${sessionIdStr}`,
            column: 'in-progress',
            board: 'main',
    ...
    const armCron = () => {
      ...
      cronTimer = setInterval(async () => {
        try {
            ...
            if (getConfig().permissionGateEnabled) {
              ...
              if ((PERMISSION_RANKS[perm] ?? 0) > (PERMISSION_RANKS[defaultPerm] ?? 0) && !task.permissionConfirmedAt) {
                ...
                store.addTransition({
                  ...
                  toCol: task.column,
                  source: 'tool',
                  detail: 'cron: запуск отклонён (требуется подтверждение повышенных прав)',
                })
                continue
              }
            }
          ...
          ctx.logger?.warn?.(`dsh-kanban: ошибка шедулера cron: ${err?.message}`)
        }
      }, 30_000)
  ...
  }), 'dsh-kanban: поток живых событий SSE')
