...
import {
  ...
  taskBody,
  taskLog, taskBySession, exportBoard, importBoard, exportTasksCsv, assignTask, boardMetrics,
  confirmPermission,
} from './routes.js'
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'POST' && action === 'cleanup-worktree') {
          const task = store.getTask(id)
          if (task === undefined) { writeJson(res, 404, { error: 'task-not-found' }); return }
          const out = await cleanupTaskWorktree({
            store,
            task,
            workspaceRegistry: ctx.get('workspaceRegistry'),
        ...
        if (req.method === 'GET' && action === 'diff') {
          const task = store.getTask(id)
          if (task === undefined) { writeJson(res, 404, { error: 'task-not-found' }); return }
          const diffResult = await getTaskDiff({ worktreePath: task.worktree })
          reply(res, diffResult)
          return
        }
        if (req.method === 'GET' && action === 'commits') {
          const task = store.getTask(id)
          if (task === undefined) { writeJson(res, 404, { error: 'task-not-found' }); return }
          const commits = await getTaskCommits({ worktreePath: task.worktree })
          reply(res, { commits })
          return
        }
        if (req.method === 'POST' && action === 'merge') {
          ...
          const mergeResult = await mergeTaskWorktree({
            task,
            targetBranch: typeof input?.targetBranch === 'string' ? input.targetBranch.trim() : undefined,
            workspaceRegistry: ctx.get('workspaceRegistry'),
          })
          if (!mergeResult.success) {
            writeJson(res, 400, { error: mergeResult.error, detail: mergeResult.detail, files: mergeResult.files })
            return
          }
          const updated = store.acceptTask(id)
          store.addTransition({
            taskId: id,
            fromCol: task.column,
            toCol: 'done',
            source: 'manual',
            detail: `принята и смержена ветка ${task.branch} в ${mergeResult.targetBranch}`,
          })
          reply(res, { task: updated, merged: true, targetBranch: mergeResult.targetBranch })
          return
        }
        if (req.method === 'GET' && action === 'worktree-status') {
          const task = store.getTask(id)
          if (task === undefined) { writeJson(res, 404, { error: 'task-not-found' }); return }
          const dirtyCheck = await checkWorktreeDirty({ worktreePath: task.worktree })
          reply(res, {
          ...
          const task = store.getTask(id)
          if (task === undefined) { writeJson(res, 404, { error: 'task-not-found' }); return }
          const list = Array.isArray(input?.checklist) ? input.checklist : Array.isArray(input) ? input : []
          const updated = store.updateChecklist(id, list)
          reply(res, { task: updated })
          return
        }
        if (req.method === 'POST' && action === 'report') {
          const task = store.getTask(id)
          if (task === undefined) { writeJson(res, 404, { error: 'task-not-found' }); return }
          const updated = store.setReport(id, input)
          reply(res, { task: updated })
          return
        }
        if (req.method === 'GET' && action === 'comments') {
          const task = store.getTask(id)
          if (task === undefined) { writeJson(res, 404, { error: 'task-not-found' }); return }
          reply(res, { comments: task.comments || [] })
          return
        }
        if (req.method === 'POST' && action === 'comments') {
          ...
          const updated = store.addComment(id, { author, role, text })
          reply(res, { task: updated })
          return
        }
        ...
        if (req.method === 'POST' && action === 'reject') {
          ...
          if (!reason) { writeJson(res, 400, { error: 'reason-required' }); return }
          const updated = store.rejectTask(id, reason)
          reply(res, { task: updated })
          return
        }
        ...
        if (req.method === 'POST' && action === 'progress-dump') {
          const task = store.getTask(id)
          if (task === undefined) { writeJson(res, 404, { error: 'task-not-found' }); return }
          const formatted = typeof input === 'string' ? input : formatProgressDump(input)
          const updated = store.updateTask(id, { progressDump: formatted })
          reply(res, { task: updated })
          return
        }
