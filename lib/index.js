...
import {
  ...
  buildBoard, applyMove, createTask, updateTask, deleteTask, appendNote, taskLog, taskBySession,
} from './routes.js'
import { listImportable, importIssue, refreshTask, searchRepos, createProjectTask } from './import.js'
import { runTask, resolveModel, buildStartMessage } from './launcher.js'
import { dispatchMove, MOVE_DETAIL } from './commands.js'
  ...
  const runSync = async () => {
    ...
    try {
      ...
      const taken = await intakeAll({
        gitea, store, config: getConfig(), owner: defaultOwner(), logger: ctx.logger,
      })
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const taken = isWatched({
          config: getConfig(), owner: target.owner, defaultOwner: defaultOwner(), repo: target.repo,
        })
          ? await intakeAll({
            gitea, store, config: getConfig(), owner: defaultOwner(), logger: ctx.logger,
