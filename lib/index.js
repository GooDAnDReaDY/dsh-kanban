...
import { boardMoveDefinition } from './board-tool.js'
...
export function apply(ctx, config) {
  ...
  ctx.effect(() => {
    ...
    const registerBoardTool = () => ctx.tools.register(defineTool(boardMoveDefinition({ store })))
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'POST' && action === 'move') {
          ...
          if (out?.task !== undefined) queueOutbound(id, out.task.column)
          reply(res, out)
          return
        }
