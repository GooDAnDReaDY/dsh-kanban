  ...
  Object.entries(CONFIG_DEFAULTS).map(([field, fallback]) => {
    ...
    let node = typeof fallback === 'number' ? z.number()
      : typeof fallback === 'boolean' ? z.boolean()
        : z.string()
    if (field === 'giteaTokenRef' || field === 'webhookSecretRef') {
  ...
  ctx.effect(() => {
    ...
    const registerBoardTool = () => {
      const off = [
        ...
        ctx.tools.register(defineTool(boardDecomposeDefinition({ store, gitea }))),
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'POST' && action === 'triage') {
          const resObj = await handleTriageTask({ store, gitea, task: store.getTask(id), input })
          writeJson(res, resObj.status, resObj.body)
          return
        }
        if (req.method === 'POST' && (action === 'subtask' || action === 'subtasks')) {
          const resObj = await handleCreateSubtask({ store, gitea, task: store.getTask(id), input })
          writeJson(res, resObj.status, resObj.body)
          return
        }
        ...
        if (req.method === 'POST' && action === 'pr') {
          const resObj = await handleCreateTaskPr({ store, gitea, task: store.getTask(id), input })
          writeJson(res, resObj.status, resObj.body)
          return
        }
