 ...
 */
export const inject = ['settings', 'webServer', 'agents', 'agentDefaultModel']
...
export function apply(ctx, config) {
  ...
  // Читатель настроек и хранилища для следующих задач: импорт и запуск берут
  // значения отсюда, а не из замороженной копии момента загрузки.
  ctx.kanbanConfig = getConfig
  ctx.kanbanStore = () => store
  ...
  }), 'dsh-kanban: маршрут создания задачи')

  ctx.effect(() => ctx.webServer.register({
    kind: 'prefix',
    path: '/dsh-kanban/task/',
    handler: async (req, res) => {
      const parsed = parseTaskPath(urlOf(req).pathname)
      ...
      if (req.method === 'POST' && action === 'refresh') {
        reply(res, await refreshTask({ gitea: ctx.gitea, store, id }))
        return
      }
  ...
  // Служба gitea необязательна и читается на каждом запросе, а не при
  // загрузке: плагин dsh-gitea может появиться позже, и до тех пор маршруты
  // обязаны внятно объяснять причину, а не отвечать 404.
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/dsh-kanban/gitea/repos',
    handler: async (req, res) => {
      const url = urlOf(req)
      reply(res, await searchRepos({ gitea: ctx.gitea, query: url.searchParams.get('q') || '' }))
    },
