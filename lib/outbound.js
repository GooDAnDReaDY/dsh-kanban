...
export function planOutbound(task, column, config) {
  ...
  if (!same) {
    ops.push({ kind: 'labels', owner: task.owner, repo: task.repo, index: task.issueNumber, labels: next })
  }
  if (column === 'done') {
        ...
        try {
          if (item.op.kind === 'labels') {
            await gitea.setLabels({
              owner: item.op.owner, repo: item.op.repo, index: item.op.index, labels: item.op.labels,
            })
