...
/**
 * Когда issue завели в Gitea.
 *
 * Без этого карточка показывала бы дату подхвата: задача годичной давности
 * выглядела бы заведённой сегодня, и по дате нельзя было бы понять ничего.
 */
export function issueBornAt(issue) {
 ...
 * Кто завёл issue.
 *
 * Берём `login`: он же стоит в Gitea под задачей и по нему её ищут. Полное
 * имя заполнено не у всех, и подставлять его вместо логина значило бы
 * показывать то одно, то другое.
 */
export function issueAuthor(issue) {
  const login = issue?.user?.login
  return typeof login === 'string' ? login : ''
}

/**
 * Кто отвечает за issue.
 *
 * Gitea отдаёт назначенного двумя способами: одиночным `assignee` и списком
 * `assignees`. Берём первого из списка, когда одиночного нет: доска показывает
 * ОДНОГО ответственного, потому что «отвечают все» означает «не отвечает
 * никто».
 */
export function issueAssignee(issue) {
  const one = issue?.assignee?.login
  if (typeof one === 'string' && one !== '') return one
  const many = Array.isArray(issue?.assignees) ? issue.assignees : []
  const first = many.find((x) => typeof x?.login === 'string' && x.login !== '')
  return first === undefined ? '' : first.login
}

/**
 * К какому выпуску относится issue.
 *
 * Релизы ведут майлстоунами Gitea, а доска о них не знала: «что осталось до
 * 0.2.0» приходилось смотреть в другом месте, хотя карточки лежат здесь.
 */
export function issueMilestone(issue) {
 ...
 */
export function issueToTask(issue, { board = 'main', column = 'backlog', owner, repo, provider }) {
  const p = provider || (typeof issue?.html_url === 'string' && issue.html_url.includes('github.com') ? 'github' : 'gitea')
  return {
    board,
    column,
    provider: p,
    ...
    labels: Array.isArray(issue?.labels)
      ...
      : [],
    // Цвета едут рядом с именами, а не вместо них: имена читают отборы,
    // сверка, чип и поиск.
    labelColors: colorsOfIssue(issue?.labels),
    owner,
    repo,
    ...
    createdAt: issueBornAt(issue),
    author: issueAuthor(issue),
    assignee: issueAssignee(issue),
    milestone: issueMilestone(issue),
  }
}
...
export function refreshPatch(task, issue) {
  ...
  if (Array.isArray(issue?.labels)) {
    ...
    patch.labelColors = colorsOfIssue(issue.labels)
  }
  if (typeof issue?.html_url === 'string') patch.issueUrl = issue.html_url
  const born = issueBornAt(issue)
  if (born !== undefined) patch.createdAt = born
  const author = issueAuthor(issue)
  if (author !== '') patch.author = author
  // Снятие ответственного — тоже факт, поэтому пустое значение пишется наравне
  // с именем: иначе доска показывала бы вчерашнего владельца задачи, с которой
  // его уже сняли.
  if (issue?.assignee !== undefined || Array.isArray(issue?.assignees)) {
    patch.assignee = issueAssignee(issue)
  }
  // Майлстоун снимают так же часто, как ставят, поэтому пустое значение —
  // тоже новость, а не отсутствие новостей.
  if ('milestone' in (issue ?? {})) patch.milestone = issueMilestone(issue)
  return patch
}
