...
export function createGiteaClient({ getConfig, resolveToken, fetchImpl = fetch, timeoutMs = 30_000 }) {
  ...
  return {
    ...
    /** Метки репозитория: имена и их идентификаторы. */
    async listLabels({ owner, repo }) {
      const rows = await request('GET', repoPath(owner, repo, '/labels?limit=100'))
      return (Array.isArray(rows) ? rows : []).map((l) => ({ id: l?.id, name: l?.name ?? '' }))
    },

    /**
     * Назначить метки по ИМЕНАМ.
     *
     * API назначения принимает идентификаторы, а не имена: запрос с именами
     * проходит с успешным кодом и не делает НИЧЕГО. Поэтому имена
     * разрешаются в идентификаторы, а недостающие метки колонок заводятся.
     *
     * Заводятся только свои метки — те, что переданы как `creatable`. Выдумывать
     * чужие метки процесса доска права не имеет.
     */
    async setLabels({ owner, repo, index, labels, creatable = [] }) {
      const n = safeSegment(index)
      if (n === undefined) throw Object.assign(new Error('недопустимый номер issue'), { code: 'bad-segment' })
      ...
      for (const name of labels ?? []) {
        if (byName.has(name)) { ids.push(byName.get(name)); continue }
        if (!creatable.includes(name)) continue
        const made = await request('POST', repoPath(owner, repo, '/labels'), {
          name, color: '#5b8def', description: 'Колонка доски dsh-kanban',
        })
        if (made?.id !== undefined) { byName.set(name, made.id); ids.push(made.id) }
      }
      return request('PUT', repoPath(owner, repo, `/issues/${n}/labels`), { labels: ids })
    },
