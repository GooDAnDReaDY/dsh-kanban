...
/**
 * Репозитории, за которыми следим, если их назвали поимённо.
 *
 * Пустой список означает «все», и разбирается это не здесь, а в проходе
 * подхвата: там есть Gitea, у которого можно спросить, какие репозитории вообще
 * существуют.
 *
...
/**
 * Следим ли за этим репозиторием.
 *
 * Событие из репозитория вне списка подхвата не заслуживает: по таймеру мы
 * туда не ходим, и через вебхук ходить тоже не должны — иначе список перестаёт
 * что-либо решать.
 */
export function isWatched({ config, owner, defaultOwner, repo }) {
  return watchedRepos({ config, owner: defaultOwner }).some(
    (pair) => pair.owner === owner && pair.repo === repo,
  )
}
