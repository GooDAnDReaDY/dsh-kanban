...
/**
 * Host-side one-click updater for @goodandready/dsh-kanban.
...
export function registerPluginUpdater(ctx, options) {
  ...
  if (typeof ctx?.webServer?.register !== 'function') {
    ctx?.logger?.warn?.('[dsh-kanban] webServer service unavailable, updater skipped')
    return () => {}
  }
  ...
  return ctx.webServer.register({
    ...
    handler: async (request, response) => {
      try {
        ...
        ctx?.logger?.warn?.(`[dsh-kanban] plugin updater failed: ${String(error)}`)
