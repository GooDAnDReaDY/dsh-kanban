 ...
 *
 * Своя задача вроде «прибраться в логах» не имеет ни репозитория, ни issue, и
 * распоряжение делать preflight веток и заводить worktree превращается для неё
 * в бессмыслицу: агент честно идёт искать репозитории и перечислять worktree
 * вместо того, чтобы заняться написанным.
 */
...
export async function startTask({
  ...
  handle.agent.followup(createMessage({
    ...
    content: [{ type: 'text', text: buildStartMessage(task, config) }],
