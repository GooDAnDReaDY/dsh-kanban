...
/**
 * Выполнить операцию с экспоненциальной задержкой (exponential backoff) при сбоях сети.
 * @template T
 * @param {() => Promise<T>} fn асинхронная операция
 * @param {{retries?: number, baseMs?: number, maxMs?: number}} [opts]
 * @returns {Promise<T>}
 */
export async function withBackoff(fn, { retries = 2, baseMs = 50, maxMs = 500 } = {}) {
  let attempt = 0
  let firstErr
  while (true) {
    try {
      ...
      if (attempt >= retries) throw (firstErr || err)
 ...
 */
async function readRepo({ gitea, owner, repo }) {
...
export async function syncAll({ gitea, store, logger, only, now = Date.now() }) {
  ...
  for (const group of byRepo.values()) {
    let repoState
    try {
      repoState = await withBackoff(() => readRepo({ gitea, owner: group.owner, repo: group.repo }), { retries: 2, baseMs: 40 })
    } catch (error) {
 ...
 */
async function resolveRepos({ gitea, config, owner }) {
