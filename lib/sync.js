...
import { watchedRepos, shouldTake, archiveBefore } from './intake.js'
import { issueToTask } from './import.js'
 ...
 */
export async function intakeAll({ gitea, store, config, owner, board = 'main', logger }) {
  const repos = watchedRepos({ config, owner })
  let added = 0
  let skipped = 0
  let failed = 0
  let problem
  ...
  for (const pair of repos) {
    ...
    for (const issue of issues ?? []) {
      ...
      store.createTask(issueToTask(issue, {
        board, column: 'backlog', owner: pair.owner, repo: pair.repo,
      }))
      added += 1
    }
  }
