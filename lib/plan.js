 ...
 *
 * `queued` — задача отдана сессии, но агент до неё не дошёл: она ждёт своей
 * очереди. Выводится из отметки постановки, а не хранится отдельным полем.
 *
...
export function taskState({ task, live }) {
  ...
  // Задача отдана агенту, но он до неё ещё не дошёл. Без этого состояния
  // десять карточек в бэклоге ничем не отличаются от нетронутых, и через час
  // никто не вспомнит, какие уже отданы.
  if (typeof task?.queuedAt === 'number' && task.queuedAt > 0) return 'queued'
...
/**
 * Номер задачи в очереди своей сессии.
 *
 * Считается на лету, а не хранится: агент берёт задачи одну за другой, и у
 * оставшихся третий сам становится вторым. Хранимый номер пришлось бы
 * пересчитывать, и однажды его бы забыли.
 *
 * Порядок — по отметке постановки, а НЕ по положению карточки в колонке.
 * Перетаскивание мышью — про то, как задачи лежат; очередь — про то, в каком
 * порядке их получит агент, и менять второе первым нельзя.
 *
 * @returns {number|undefined} место начиная с единицы
 */
export function queuePosition(task, sameSession) {
  const at = task?.queuedAt
  if (typeof at !== 'number' || at <= 0) return undefined
  const ahead = (sameSession ?? [])
    .filter((x) => typeof x?.queuedAt === 'number' && x.queuedAt > 0 && x.queuedAt < at)
