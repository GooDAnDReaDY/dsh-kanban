...
const WRITABLE = [
  ...
  'branch', 'worktree', 'sessionId', 'model', 'provider',
]
...
function rowToTask(row) {
  ...
  return { ...rest, column: col, labels: parsed }
}
  ...
  return {
    ...
    createTask(input) {
      ...
      db.prepare(`INSERT INTO tasks
        (id, board, col, position, title, body, owner, repo, issueNumber, issueUrl,
         labels, branch, worktree, sessionId, model, provider, createdAt, updatedAt)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
    ...
    updateTask(id, patch) {
      ...
      for (const key of WRITABLE) {
        ...
        fields.push(`${key} = ?`)
        values.push(key === 'labels' ? JSON.stringify(patch[key] ?? []) : patch[key] ?? null)
      }
      if (fields.length > 0) {
