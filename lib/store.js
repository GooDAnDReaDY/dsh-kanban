...
const WRITABLE = [
  ...
  'branch', 'worktree', 'sessionId', 'model', 'provider', 'waiting', 'syncedAt',
]
...
function rowToTask(row) {
  ...
  const { col, labels, waiting, ...rest } = row
  let parsed = []
  try {
  ...
  return { ...rest, column: col, labels: parsed, waiting: waiting === 1 }
}
...
export function openStore({ dir }) {
  ...
  for (const [column, definition] of [['waiting', 'INTEGER NOT NULL DEFAULT 0'], ['syncedAt', 'INTEGER NOT NULL DEFAULT 0']]) {
  ...
  const q = {
    ...
    insertBoard: db.prepare('INSERT INTO boards (id, title, position, createdAt) VALUES (?, ?, ?, ?)'),
    ...
    tasksByBoard: db.prepare('SELECT * FROM tasks WHERE board = ? ORDER BY col, position'),
  ...
  function ensureBoard(id, title) {
    const found = q.boardById.get(id)
    if (found !== undefined) return found
    const last = q.lastBoard.get()
    const position = keyBetween(last?.position, undefined)
    q.insertBoard.run(id, title, position, Date.now())
    return q.boardById.get(id)
  }

  // Доска по умолчанию заводится сразу: пустое хранилище без единой доски
  // выглядит в интерфейсе как поломка, а не как чистый старт.
  ensureBoard('main', 'Основная доска')
  ...
  return {
    ...
    createBoard({ id, title }) {
      return ensureBoard(id, title ?? id)
    },
    ...
    createTask(input) {
      ...
      db.prepare(`INSERT INTO tasks
        (id, board, col, position, title, body, owner, repo, issueNumber, issueUrl,
         labels, branch, worktree, sessionId, model, provider, waiting, syncedAt, createdAt, updatedAt)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
    ...
    moveTask(id, { column, beforeId, afterId }) {
      ...
      db.prepare('UPDATE tasks SET col = ?, position = ?, updatedAt = ? WHERE id = ?')
        .run(target, position, Date.now(), id)
      return rowToTask(q.taskById.get(id))
    },
