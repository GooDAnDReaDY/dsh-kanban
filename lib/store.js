...
const WRITABLE = [
  ...
  'branch', 'worktree', 'sessionId', 'model', 'provider', 'waiting',
]
...
export function openStore({ dir }) {
  ...
  for (const [column, definition] of [['waiting', 'INTEGER NOT NULL DEFAULT 0']]) {
    ...
    createTask(input) {
      ...
      db.prepare(`INSERT INTO tasks
        (id, board, col, position, title, body, owner, repo, issueNumber, issueUrl,
         labels, branch, worktree, sessionId, model, provider, waiting, createdAt, updatedAt)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
