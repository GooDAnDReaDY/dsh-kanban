...
import { COLUMN_ORDER, wipLimitField } from './config.js'
 ...
 */
export function buildBoard({ store, config, board = 'main', repo }) {
  const tasks = store.listTasks({ board, repo })
  ...
  return { board, boards: store.listBoards(), columns, tasks }
}
...
export function createTask({ store, input }) {
  ...
  const column = COLUMN_ORDER.includes(input?.column) ? input.column : 'backlog'
...
export function taskBySession({ store, sessionId }) {
  return { task: store.findTaskBySession(sessionId) ?? null }
}
